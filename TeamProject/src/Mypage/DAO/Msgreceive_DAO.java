...
public class Msgreceive_DAO {
	...
	//DB연결
		...
		// 초기자 { } static 초기자 : static{ }
			...
			//받은 쪽지 갯수 반환
			public int getmsgcount(String userid) throws SQLException{
				try {
					conn = ds.getConnection();
					String sql = 
					"select count(*) from message where M_RECIEVEID=?";
					int count = 0;
			...
			//id값 비교해서 목록 불러오기
			public ArrayList<Message_DTO> getrecievecelist(String userid) throws SQLException{
				try {
					conn = ds.getConnection();
					String sql = 
					"select * from message where M_RECIEVEID=? ORDER BY M_DATE desc";
					ArrayList<Message_DTO> rmsg = new ArrayList<Message_DTO>();// 받은 메시지 리스트
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1,userid);
					rs = pstmt.executeQuery();
