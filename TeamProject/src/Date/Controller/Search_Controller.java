...
public class Search_Controller extends HttpServlet {
   ...
   private void Process(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{   
      ...
      }else if(cmd.equals("/friendlist.sear")){   
         try {
            PrintWriter out = response.getWriter();
            
            
            HttpSession session = request.getSession();
            String u_id = request.getParameter("sendid");
            
            System.out.println("controller : "+u_id);
            ArrayList<Search_DTO> arr = new ArrayList<Search_DTO>();
            
            Search_DAO friendlist = new Search_DAO();
            
            arr = friendlist.getFriendList(u_id);
            
            JSONArray seaelist = JSONArray.fromObject(arr);
            response.getWriter().print(seaelist);//서버로 데이터 전송
            System.out.println("서버로 list 전송완료");
         } catch (Exception e) {
            e.printStackTrace();
